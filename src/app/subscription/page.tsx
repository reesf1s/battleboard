...
export default function SubscriptionPage() {
  if (isDemoMode()) return <SubscriptionPageInner convexUser={null} />;
  return <SubscriptionPageReal />;
}

function SubscriptionPageReal() {
  const { convexUser } = useCurrentUser();
  return <SubscriptionPageInner convexUser={convexUser} />;
}

function SubscriptionPageInner({ convexUser }: { convexUser: any }) {
  ...
  const hasActiveSubscription =
    ...
    (convexUser.subscriptionStatus === "active" ||
      (convexUser.subscriptionStatus === "trial" &&
        (!convexUser.subscriptionExpiresAt || convexUser.subscriptionExpiresAt > Date.now())));
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-12 max-w-sm mx-auto w-full">
      {/* Back — only show if user has an active subscription (can return to dashboard) */}
      ...
      {/* Header */}
      <div className={cn("text-center mb-8", !hasActiveSubscription && "mt-4")}>
        ...
        <h1 className="app-display text-[22px] font-bold text-foreground mb-1.5">
      ...
      <div className="flex flex-col gap-4 mb-6">
        {PLANS.map((plan) => {
          const isCurrentPlan = convexUser?.subscriptionTier === plan.id;
          return (
            <Card
              key={plan.id}
              className={cn(
                "gap-0 py-0 overflow-hidden relative",
                plan.popular && !isCurrentPlan && "ring-1 ring-primary/30",
                isCurrentPlan && "ring-1 ring-primary/50",
              )}
            >
              {isCurrentPlan ? (
              ...
              <div className="p-4">
                ...
                {isCurrentPlan ? (
                  <Button
                    disabled
                    className="w-full bg-white/[0.06] text-muted-foreground border border-white/[0.08]"
                    size="lg"
                  >
                    Current Plan
                  </Button>
